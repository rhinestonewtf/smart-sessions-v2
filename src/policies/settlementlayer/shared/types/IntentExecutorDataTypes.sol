...
/// @notice Variant tag at byte 0 of the policy data blob.
uint8 constant VARIANT_SINGLE_CHAIN = 0;
uint8 constant VARIANT_MULTI_CHAIN = 1;

/// @notice Maximum number of (to, value, data) calls allowed inside a signed Operation.
/// @dev Bounds calldata walking gas. Picked to comfortably cover real Relay/CCTP/Rhino fills.
uint256 constant MAX_OPS = 16;

/// @notice Hard cap on raw calldata length of an individual call inside Operation.ops.
...
/// @dev Matches `EIP712Lib.NO_GASREFUND`: `keccak256(abi.encode(TYPEHASH_GAS_REFUND, address(0), 0))`.
bytes32 constant NO_GASREFUND =
    0x2a2e93882f19103eba0384726c9f0fc4688a6ecb1e90bef28a42ac83bffe95d8;

/// @notice Compact configuration flags stored as a single uint8.
